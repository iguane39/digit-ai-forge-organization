# Digit-AI — Gabarit Forge — A0 « Page HTML de restitution » — 20260809b
...
**Version 20260809a** — ajout de la **Partie 1 bis, lisibilité (A0-13 à A0-22)**, dérivée
des dix défauts relevés par un lecteur humain sur un livrable réel le 09/08/2026. Aucun
n'était détecté par les oracles en place : `check_html`, `render_page` et l'oracle de
filtres étaient tous les trois au vert sur une page dont les textes étaient coupés, les
scores sans barème, le sommaire muet et les chapitres de données sans mode d'emploi. La
version `20260808a` est archivée dans `Old/`.

**Version 20260809b** — **A0-23 à A0-25**, issues d'une seconde lecture, cette fois par un
lecteur naïf, sur le livrable *déjà corrigé* : verdict « oui, mais ». Les onze règles de la
version précédente étaient toutes au vert. L'enseignement vaut d'être inscrit ici plutôt que
dans un commit : **le premier tour d'exigences ne trouve que ce qu'il sait chercher.** Ce
sont des lecteurs, pas des règles, qui trouvent les règles suivantes — et une famille de
gabarits qui ne se rouvre pas après chaque lecture réelle se fige sur ses angles morts.
...
(règles L1–L11 de `digit-ai-page-html/references/lisibilite.md`, contrôlées par
`check_html.py --regles L` et prouvées faillibles par `scripts/self_test.py`). Une page peut
être exacte, chartée, sans défaut visuel — et illisible.
...
mécanique dans `check_html.py --regles L` et une fixture rouge dans `fixtures/` du skill :
une règle sans fixture rouge n'est pas un contrôle, c'est une déclaration d'intention.
État au 09/08/2026 : **17 fixtures, 16 rouges**, chacune exigeant l'ensemble exact des
règles attendues — ni moins, le contrôle serait aveugle, ni plus, il serait bruyant.

| ID | Pattern | Exigence | Critère vérifiable | Source |
|---|---|---|---|---|
| **A0-13** | `Si … alors` | Si un texte doit être raccourci, alors il s'arrête à une frontière grammaticale ; il n'est jamais amputé. | `L1` — zéro `…` en fin de nœud de texte hors élément `data-ellipse-ok`. Le niveau 1 d'une strate porte une unité de sens complète, le niveau 2 le texte intégral. | Défaut 1 du 09/08/2026 : « Sur deux des… », coupé au niveau 1 **et** au niveau 2 |
| **A0-14** | — | La page occupe la largeur disponible ; seule la prose est bornée. | `L2` — aucun `max-width` en pixels durs < 1 100 px sur le conteneur. Deux largeurs : conteneur `min(92vw,1680px)`, prose ~75 ch. | Défaut 2 : 1 080 px en dur, 44 % de surface perdue en 1 920 |
| **A0-15** | — | Tout score affiché renvoie à un barème publié dans la page. | `L3` — élément de classe `sc`/`score` ou au texte `N/M` : `aria-describedby` résolvant vers un élément de 20 caractères au moins — le barème. Un `title` ne suffit pas : il ne survit pas au PDF et n'a pas la place d'énoncer cinq crans. | Défaut 3 : « maturité 1/5 » sans dire ce que valent 1 à 5 |
| **A0-16** | — | Toute autre valeur mise en avant porte une légende non vide. | `L3` — `kpi`/`badge`/`pastille` : `title`, `aria-label`, `aria-describedby` ou légende visible de 12 caractères au moins. Une légende **vide** est un échec, pas un avertissement : elle annonce une explication et n'en donne aucune. | Défaut 3 |
...
| **A0-20** | — | Chaque chapitre ouvre par ce qu'il apprend au lecteur. | `L7` — toute section cible du sommaire porte un `.ch-apprend` de 40 caractères au moins, avant toute donnée. | Défaut 10 : chapitres ouvrant sur un tableau de 200 lignes |
...
| **A0-22** | `Lorsque` | Lorsqu'un chapitre porte une table de 8 lignes ou plus, il donne un exemple de lecture d'une ligne. | `L10` — élément `.exemple-lecture` de 30 caractères au moins dans la section. Complément : `L9`, aucun `<details>` au corps vide. | Défaut 10 : « ni macro ni mode d'emploi » |
...
linéaire, fidélité des libellés de lien. Un run qui déclare « lisibilité OK » sans énumérer
ce qui a été relu n'a pas fait de revue.
...
`Digit-AI - Audit - SEO auxportesdelabaie.fr - 20260808g.html`, seule source de la
Partie 1 bis, complétée le même jour par une lecture naïve du livrable corrigé
(A0-23 à A0-25). Chaque exigence est rattachée au défaut qui l'a payée.

**Internes** (test d'existence : lues sur cette machine le 08/08/2026)
- `digit-ai-page-html` — SKILL.md 1.3.0, cibles Viewer/PDF, checklist V1–V7,
  `references/lisibilite.md` (L1–L11), `scripts/self_test.py` (17 fixtures, 16 rouges)
